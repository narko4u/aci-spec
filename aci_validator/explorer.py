#!/usr/bin/env python3
"""ACI Explorer — Discover and display ACI manifests for any domain.

Usage:
    aci-explore example.com
    aci-explore https://example.com

A zero-dependency tool (stdlib only) for exploring an organization's
Autonomous Company Interface manifests.
"""
...
def _extract_discovery_urls(data):
    """Extract all URLs from a manifest's 'discovery' field."""
    urls = set()
...
def show_identity(data, url):
    ...
    print(_fmt_subheader("Identity Manifest"))
...
def discover_and_display(target):
    ...
    while pending:
        ...
        if data and not err:
            ...
            all_manifest_urls.discard(murl)
    ...
    if manifest_count == 5:
        ...
        val_url = base_url
        print(f"  {_c('dim', 'Formal conformance')}: run {_c('cyan', 'aci-validate ' + val_url)}")
...
def main():
    if len(sys.argv) < 2:
        ...
        print(f"    aci-explore <domain-or-url>")
        print()
        print(f"  {_c('bold', 'Examples')}:")
        print(f"    aci-explore example.com")
        print(f"    aci-explore https://example.com")
        print()
        sys.exit(1)
