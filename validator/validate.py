...
Usage:
    python3 validate.py https://example.com
    python3 validate.py https://example.com --verbose
    python3 validate.py https://example.com --json   (machine-readable output)

Schemas are loaded dynamically from schema/*.yaml at startup.
Conformance levels (1-3) are detected and reported per SPEC.md §3.
"""
...
def validate_aci(target_url: str) -> ACIValidationResult:
    ...
    for mtype in MANIFEST_ORDER:
        ...
        # Strip x- extension keys before validation (they carry no normative weight)
